...
def main():
    ...
    # for every n 
    for k in n_list:
        # generate an array 'A' from 0 to n, but only fill the list with even numbers
        A = list(range(0,k*2,2))
        # generate a second array that includes ONLY the numbers 1 before, 1 after, and in between values of array 'A'
        B = list(range(-1,k*2+2,2))
        # keep track of total comparisons for bin and trin searches
        total_bin = 0
        total_trin = 0
        # for every value in the array 'B', perform a trinary and binary search for the value in 'A'
        for i in B:
            # store the total amount of comparisons made in 'A' for every value in 'B'
            total_bin += bin_search(A,i) 
            total_trin += trin_search(A,i)
        # for every total comparisons made with 'A', divide by the total searches made with 'B'
        bin_avg.append(total_bin/len(B))
        trin_avg.append(total_trin/len(B))
